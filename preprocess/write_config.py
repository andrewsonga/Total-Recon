...
#seqname_all = sorted(glob.glob('%s/%s[0-9][0-9][0-9]'%(silroot, seqname_pre)))         # only includes videos with seqnames that exactly follow this template, without any trailing substrings
seqname_all = sorted(glob.glob('%s/%s[0-9][0-9][0-9]*'%(silroot, seqname_pre)))         # also includes videso with seqnames that start with the template, but also with trailing substrings e.g. -leftcam, -rightcam
##########################################################################################
##########################################################################################
