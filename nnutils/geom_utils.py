...
def bone_transform(bones_in, rts, is_vec=False):
    ...
    #rts are transformation applied to bone coordinates (right multiply)    #manual update (04/11 commit from banmo repo)
    rts are applied to bone coordinate transforms (left multiply)           #manual update (04/11 commit from banmo repo) 
    ...
    #center = transforms.quaternion_to_matrix(orient).matmul(Tmat)[...,0]+center        #manual update (04/11 commit from banmo repo)
    # move bone coordinates (left multiply)                                             #manual update (04/11 commit from banmo repo)
    center = Rmat.matmul(center[...,None])[...,0]+Tmat[...,0]                           #manual update (04/11 commit from banmo repo)
    Rquat = transforms.matrix_to_quaternion(Rmat)
    #orient = transforms.quaternion_multiply(orient, Rquat)                             #manual update (04/11 commit from banmo repo)
    orient = transforms.quaternion_multiply(Rquat, orient)                              #manual update (04/11 commit from banmo repo)
...
def gauss_mlp_skinning(xyz, embedding_xyz, bones, 
    ...
    """
    N_rays = xyz.shape[0]
    #if pose_code.dim() == 2:                                                           #manual update (04/11 commit from banmo repo)
    #TODO hacky way to make code compaitible with noqueryfw                             #manual update (04/11 commit from banmo repo)
    if pose_code.dim() == 2 and pose_code.shape[0]!=N_rays:                             #manual update (04/11 commit from banmo repo)
...
#def blend_skinning(bones, rts, skin, pts):
    """
    bone: bs,B,10   - B gaussian ellipsoids
    #rts: bs,B,3,4   - B ririd transforms, applied to bone coordinates                                                          #manual update (04/11 commit from banmo repo)
    rts: bs,B,3,4   - B ririd transforms, applied to bone coordinates (points attached to bones in world coords)                #manual update (04/11 commit from banmo repo)
    pts: bs,N,3     - N 3d points
    skin: bs,N,B   - skinning matrix
    ...
    B = rts.shape[-3]
    N = pts.shape[-2]
    #bones = bones.view(-1,B,10)                                #manual update (04/11 commit from banmo repo)
    pts = pts.view(-1,N,3)
    rts = rts.view(-1,B,3,4)
    Rmat = rts[:,:,:3,:3] # bs, B, 3,3
    Tmat = rts[:,:,:3,3]
...
# saves inside 'rays': 'rays_o', 'rays_d', 'near', 'far', 'rtk_vec' in the object space
# so if we provide a different set of 'Rmat' and 'Tmat's, we'll get back rays in a different object space
def raycast(xys, Rmat, Tmat, Kinv, near_far):
    """
    assuming xys and Rmat have same num of bs
    xys: bs, N, 3
    Rmat:bs, ...,3,3 
    Tmat:bs, ...,3, camera to root coord transform # actually Rmat, Tmat denotes the world-to-cam transformations
    Kinv:bs, ...,3,3 
    near_far:bs,2
    """
...
def sample_xy(img_size, bs, nsample, device, return_all=False, lineid=None):
    ...
    img_coords = torch.arange(0, img_size, dtype=torch.float32, device=device)
    xygrid = torch.cartesian_prod(img_coords, img_coords)
    xygrid = torch.unsqueeze(torch.flip(xygrid, dims=(1,)), dim=0)
    ...
    if return_all:
        ...
        if lineid is None:
            ...
            #xys_mod = xygrid[0][rand_inds[:bs]]
            #print("xys == xys_mod: {}".format(torch.mean((xys == xys_mod).float())))
        else:
...
def warp_fw(opts, model, rt_dict, vertices, embedid):
    ...
    if opts.flowbw:
        ...
        #time_embedded = model.pose_code(query_time)[:,0]   # shape = (N, )
        time_embedded = model.pose_code(query_time)         # shape = (N, 128)   
        #############################################################
        #############################################################
...
def vrender_flo(weights_coarse, xyz_coarse_target, xys, img_size):
    ...
    ##################################################################
    ################## modified by Chonghyuk Song ####################
    flo_valid = (invalid_ind.sum(-1)==0).float()[...,None]                 # only use pixel if all points along ray is valid
    #flo_valid = ((1-invalid_ind.float()).sum(-1)>0).float()[...,None]     # use pixel if at least one point along ray is valid
    ##################################################################
    ##################################################################
