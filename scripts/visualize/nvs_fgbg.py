...
def main(_):
        ...
        if opts.render_cam:
            ...
            if opts.thirdperson_view:
                ...
                try:
                    ...
                    mesh_asset.visual.vertex_colors = np.tile(np.array([[0, 0, 0.5, 1.]]), (mesh_asset.visual.vertex_colors.shape[0], 1))
            elif opts.firstperson_view:
                # color yellow
                try:
                    ...
                    mesh_asset.visual.vertex_colors = np.tile(np.array([[0.5, 0.5, 0, 1.]]), (mesh_asset.visual.vertex_colors.shape[0], 1))
    ...
    if opts.filter_3d:
        ...
        # rotate the unicorn horn -90 degrees abt x-axis to make unicorn horn compliant with OpenGL convention (where the normal / principal viewing direction points towards -ve z-ax
        rotation_abt_xaxis = np.array([[1, 0, 0],
                               [0,  np.cos(np.pi / 2), np.sin(np.pi / 2)],
                               [0, -np.sin(np.pi / 2), np.cos(np.pi / 2)]])
        mesh_asset.vertices = np.matmul(rotation_abt_xaxis, mesh_asset.vertices.T).T
        ...
        for i, frame_idx in enumerate(sample_idx):    
            ...
            # assume unicorn horn and camera both have OpenGL convention (where the normal / principal viewing direction points towards -ve z-axis)
            # extracting Z-axis direction of the 3d asset = vertex normal
            asset2fg[:3, 2] = -mesh_fg_time.vertex_normals[opts.fg_normalbase_vertex_index, :].copy()           # unit length normal vector: in OpenGL, z-axis is in opposite directio
    ...
    sils_gt_refcam_raw_objs = {obj_index: [] for obj_index in range(len(opts_list))}
    ...
    sils_gt_secondcam_raw_objs = {obj_index: [] for obj_index in range(len(opts_list))}
    ...
    for i, frame_index in enumerate(sample_idx):
        ...
        if opts.evaluate:
            ...
            if opts.stereo_view or opts.input_view:
                ...
                depth_acc_at_10cm = compute_depth_acc_at_10cm(dph_gt, dph, conf_gt, mask=None, dep_scale=opts_list[-1].dep_scale)
                ...
                depth_acc_at_10cms.append(depth_acc_at_10cm)
                ...
                for obj_index, sil_gt_obj in enumerate(sil_gt_objs):
                    ...
                    depth_acc_at_10cm_obj = compute_depth_acc_at_10cm(dph_gt, dph, conf_gt, mask = mask_obj, dep_scale=opts_list[-1].dep_scale)

                    rgb_errors_objs[obj_index].append(rgb_error_obj)
                    depth_errors_objs[obj_index].append(depth_error_obj)
                    depth_acc_at_10cms_objs[obj_index].append(depth_acc_at_10cm_obj)
        ...
        for obj_index, sil_gt_refcam in enumerate(sil_gt_refcam_objs):
            ...
            sils_gt_refcam_raw_objs[obj_index].append(sil_gt_refcam.copy())
        ...
        if opts.stereo_view:
            ...
            for obj_index, sil_gt_secondcam in enumerate(sil_gt_secondcam_objs):
                ...
                sils_gt_secondcam_raw_objs[obj_index].append(sil_gt_secondcam.copy())
