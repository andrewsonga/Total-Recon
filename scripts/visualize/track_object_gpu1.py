...
flags.DEFINE_float('dep_scale', 0.2,
...
def main(_):
    ...
    loadname_objs = ["logdir/{}/obj0/".format(opts.seqname), "logdir/{}/obj1/".format(opts.seqname)]
    rootdir_objs = ["logdir/{}/obj0/".format(opts.seqname), "cam-files/human-dualrig-fgbg002/human-dualrig-fgbg002-leftcam"]
    ...
    mesh_rest_bkgd = trimesh.load('%s/mesh-rest.obj'%(loadname_objs[-1]),process=False)
    mesh_rest_bkgd.visual.vertex_colors[:, :3] = 128                    # color = gray 
    mesh_rest_fg = trimesh.load('%s/mesh-rest.obj'%(loadname_objs[0]),process=False)
    ...
    for opts_obj in opts_list:
        ...
        if opts_obj.rootdir.startswith("cam-files"):
            rtks[:, :3, 3] *= opts_obj.dep_scale
        
        rtks_objs.append(rtks)

    # for bkgd, let's match to the same starting point as in "human-dualrig002-leftcam-ds-load-fgbgkd"
