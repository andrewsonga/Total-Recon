...
def main(_):
    ...
    for i, frame_idx in enumerate(sample_idx):    
        ...
        for obj_index, opts_obj in enumerate(opts_list):
            ...
            if obj_index == len(opts_list) - 1 and opts.prealign:
                bkgd_meshes.append(verts_obj_time[0,:,:3])            
    ...
    for i, frame_index in enumerate(sample_idx):
        ...
        if opts.prealign:
            ...
            bkgd_mesh_verts_time = bkgd_meshes[i]        # shape = (num_verts, 3) current frame index is given by "i"
            focal_refcam = rtks_objs[-1][i, 3, :2]
            ppoint_refcam = rtks_objs[-1][i, 3, 2:]
            P_valid = bkgd_mesh_verts_time / opts_list[-1].dep_scale
            
            # b) compute pointcloud based on gt depth:
            #    backproject the gt, stereo-view depth to the stereo view camera space
            P_gt = np.repeat(dph_gt[..., np.newaxis], 3, axis=-1) * np.matmul(p_homogen, np.repeat(np.linalg.inv(K_secondcam.T)[np.newaxis, ...], height, axis = 0)) / opts_list[-1].dep_scale       # (H, W, 3); for np.matmul, if either argument is N-D, it's treated as a stack of matrices residing in the last two indexes
            ...
            x_coord_valid_gt = x_coord[(conf_gt > 1.5) & (sil_gt_secondcam_objs[-1] == 1)]                                          # (H, W)[(H, W)]                                                             
            y_coord_valid_gt = y_coord[(conf_gt > 1.5) & (sil_gt_secondcam_objs[-1] == 1)]                                          # (H, W)[(H, W)]
            
            # if num. of remaining points is non-zero 
            if not np.isnan(x_coord_valid_gt):
                ...
                frts = pytorch3d.ops.iterative_closest_point(P_valid, P_gt_valid, \
                        estimate_scale=False,max_iterations=100)

                prealign_srt = torch.eye(4).cuda()
                prealign_srt[:3, :3] = frts.RTs.s * frts.RTs.R
                prealign_srt[:3, :] = frts.RTs.T

                # offset camera poses for all objects by prealign_srt
                for obj_index in range(len(rtks_objs)):
                    rtks_objs[obj_index][i, ...] = np.matmul(prealign_srt, rtks_objs[obj_index][i, ...])            # (4, 4) = (4, 4) * (4, 4)
