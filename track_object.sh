...
startframe=310
maxframe=321                      # setting this value to -1 means that we intend to use all frames of raw video (a positive integer value indicates we will either upsample or downsample the number frames)
scale=1
scale_rgbimages=0.5
...
topdowncam_offset_z=0.05           # +ve topdowncam_offset_z pushes the image downwards in the image plane
...
CUDA_VISIBLE_DEVICES=$dev python scripts/visualize/track_object.py \
                  --seqname $seqname \
