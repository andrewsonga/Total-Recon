...
#flags.DEFINE_string('loadname_fg', '', 'name of folder inside \logdir to load into fg banmo object')
#flags.DEFINE_string('loadname_bkgd', '', 'name of folder inside \logdir to load into bkgd banmo object')
opts = flags.FLAGS
...
def main(_): 
    ...
    for obj_index, loadname in enumerate(loadname_objs):
        ...
        #opts_obj.append_flag_values(opts_obj)
        #print("OPTS_OBJ use_embed: {}".format(opts.get_flag_value('use_embed', True)))
        ...
        #opts_obj.use_proj = opts.use_proj               # disabled when we want to use the "use_proj" flags from the original pretrained models
        ...
        #opts_obj.anneal_freq = opts.anneal_freq
        #opts_obj.alpha = opts.alpha
        #opts_obj.warmup_shape_ep = opts.warmup_shape_ep
        #opts_obj.warmup_rootmlp = opts.warmup_rootmlp
        #opts_obj.warmup_steps = opts.warmup_steps
        #opts_obj.nf_reset = opts.nf_reset
        #opts_obj.bound_reset = opts.bound_reset
        #opts_obj.dskin_steps = opts.dskin_steps
        #opts_obj.fine_steps = opts.fine_steps
        ...
        #opts_obj.sil_filter = opts.sil_filter          # no longer using this flag since updating to 04/11 versino of banmo repo
        ...
        if obj_index < len(loadname_objs) - 1:          # obj_index == len(loadname_objs) - 1 -> bkgd
            ...
            # opts_obj.freeze_shape = opts.freeze_shape  # for our complete model on the catpika-dualrig001 sequence

            opts_obj.freeze_root = opts.freeze_root      # for our complete model
            opts_obj.ks_opt = opts.ks_opt
            # opts_obj.freeze_root = True                # for ablated method (variant 3, 4: freezing camera pose)
            # opts_obj.ks_opt = False                    # for ablated method (variant 3, 4: freezing intrinsics)
    ...
    #print('world size: %d/ local rank %d'%(world_size,opts.local_rank))
    #########################################################################################################
    #########################################################################################################
